...
REWORK_LANGUAGE_PATTERN = re.compile(r"(#rework|\bfix(?:es|ed)?\b|\bpatch(?:es|ed)?\b|\brestore(?:s|d)?\b)", re.IGNORECASE)
...
def get_overlapping_files(source_files: list[PullRequestFile], followup_files: list[PullRequestFile]) -> list[PullRequestFile]:
    if not (validate_files_belong_to_same_pr(source_files) and validate_files_belong_to_same_pr(followup_files)):
