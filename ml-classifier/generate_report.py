...
Usage:
    cd ml-classifier
    ../backend/.venv/bin/python generate_report.py
"""
...
def plot_roc_curves(models: dict, X_test, y_test, out_path: Path) -> dict[str, float]:
    ...
    for name, model in models.items():
        ...
        RocCurveDisplay.from_predictions(
            y_test, proba, name=name,
            ax=ax, curve_kwargs={"color": MODEL_COLORS[name], "linewidth": 2},
        )
...
def plot_pr_curves(models: dict, X_test, y_test, out_path: Path) -> dict[str, float]:
    ...
    ax.axhline(baseline, linestyle="--", color="gray", linewidth=1, label=f"Baseline ({baseline:.4f})")
    ax.set_title("Precision-Recall Curve — real held-out test set\n(the metric that matters most at this class imbalance)")
...
def write_report(
    ...
    for name in roc_aucs:
        ...
        lines.append(f"| {name}{marker} | {roc_aucs[name]:.4f} | {ap_scores[name]:.4f} |")
